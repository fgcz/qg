    ...
    chart = (
        ...
        .encode(
            ...
            color=alt.Color(
                ...
                title="Sample Type",
                scale=alt.Scale(
                    domain=["sample", "qc", "clean", "blank"], range=["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728"]
                ),
            ),
            tooltip=["instrument", "sample_type", "len"],
        )
        .properties(width=500, height=300, title="Sample Types by Instrument")
    )
    chart
    return
...
def _(df, mo, pl):
    ...
    vial_analysis = (
        df.filter(pl.col("vial").is_not_null() & (pl.col("vial") != ""))
        .group_by("instrument")
        .agg(
            [
...
def _(df, mo, pl):
    ...
    qc_summary = (
        qc_samples.group_by(["instrument", "qc_type"]).len().sort(["instrument", "len"], descending=[False, True])
    )
...
def _(df, mo):
    instrument_filter = mo.ui.dropdown(
        options=["All"] + df["instrument"].unique().sort().to_list(), value="All", label="Instrument"
    )
    sample_type_filter = mo.ui.dropdown(
        options=["All"] + df["sample_type"].unique().drop_nulls().sort().to_list(), value="All", label="Sample Type"
    )
...
def _(df, instrument_filter, mo, pl):
    ...
    method_analysis = (
        method_df.filter(pl.col("method").is_not_null())
        .with_columns([pl.col("method").str.extract(r"C:\\Xcalibur\\methods\\([^\\]+)", 1).alias("method_root")])
        .group_by("method_root")
        .len()
        .sort("len", descending=True)
        .head(15)
    )

    mo.vstack([mo.md(f"**Top method roots for {inst}:**"), mo.ui.table(method_analysis)])
    return
...
def _(df, instrument_filter, mo, pl):
    ...
    output_analysis = (
        output_df.filter(pl.col("output_path").is_not_null())
        .with_columns(
            [
                pl.when(pl.col("output_path").str.contains(r"\\orders\\"))
                .then(pl.lit("orders"))
                .otherwise(pl.col("output_path").str.extract(r"\\(p\d+)\\", 1))
                .alias("project_type")
            ]
        )
    ...
    mo.vstack([mo.md(f"**Output path distribution for {inst2}:**"), mo.ui.table(output_analysis)])
    return
...
def _(df, mo, pl):
    ...
    first_by_inst = (
        first_samples.group_by(["instrument", "sample_type"])
        .len()
        .sort(["instrument", "len"], descending=[False, True])
    )

    # Last sample of each queue
    last_samples = df.sort("row").group_by("source_file").last()
    last_by_inst = (
        last_samples.group_by(["instrument", "sample_type"]).len().sort(["instrument", "len"], descending=[False, True])
    )
...
def _(df, mo, pl):
    ...
    _first_qc = (
        ...
        .with_columns([pl.col("filename").str.extract(r"(autoQC\d*[a-zA-Z]*)", 1).alias("qc_type")])
    )
...
def _(df, mo, pl):
    ...
    blank_by_inst = (
        blanks.group_by("instrument")
        .agg([pl.len().alias("count"), pl.col("filename").first().alias("example_filename")])
        .sort("instrument")
    )
...
def _(blanks, df, mo, pl):
    ...
    if _positions:
        blank_positions = (
            pl.DataFrame(_positions)
            .group_by(["instrument", "position"])
            .len()
            .sort(["instrument", "len"], descending=[False, True])
        )
        mo.vstack(
...
def _(alt, df, mo, pl):
    ...
    _queue_counts = _df_filtered.group_by("instrument").agg(pl.col("source_file").n_unique().alias("n_queues"))
    ...
    _start_melted = start_patterns.unpivot(
        index=["position", "instrument"], on=["qc", "clean", "blank"], variable_name="sample_type", value_name="pct"
    )

    start_chart = (
        alt.Chart(_start_melted.to_pandas())
        .mark_bar()
        .encode(
            ...
            color=alt.Color(
                "sample_type:N",
                title="Type",
                scale=alt.Scale(domain=["qc", "clean", "blank"], range=["#ff7f0e", "#2ca02c", "#d62728"]),
            ),
            column=alt.Column("instrument:N", title="Instrument"),
            tooltip=["instrument", "position", "sample_type", "pct"],
        )
        .properties(width=120, height=200, title="Queue Start Patterns (% of queues with >10 samples)")
    )

    mo.vstack(
        [
            mo.md(
                f"**Queues with >10 samples:** {', '.join([f'{k}={v}' for k, v in sorted(_queue_count_dict.items())])}"
            ),
            start_chart,
...
def _(alt, df, mo, pl):
    ...
    end_chart = (
        alt.Chart(_end_melted.to_pandas())
        .mark_bar()
        .encode(
        ...
        .properties(width=120, height=200, title="Queue End Patterns (% of queues with >10 samples)")
    )
...
def _(df, mo, pl):
    ...
    _df_qc = df.with_columns([pl.col("filename").str.extract(r"(autoQC\d*[a-zA-Z]*)", 1).alias("qc_type")])

    _qc_start = _df_qc.filter((pl.col("run") == 1) & (pl.col("sample_type") == "qc"))
    qc_start_summary = (
        _qc_start.group_by(["instrument", "qc_type"]).len().sort(["instrument", "len"], descending=[False, True])
    )
    ...
    qc_end_summary = (
        _qc_end.group_by(["instrument", "qc_type"]).len().sort(["instrument", "len"], descending=[False, True])
    )
...
def _(alt, df, mo, pl):
    ...
    gap_df = (
        _df_filtered.filter(pl.col("sample_type").is_in(["qc", "clean"]))
        .sort(["instrument", "source_file", "run"])
        .with_columns([pl.col("run").diff().over(["instrument", "sample_type"]).alias("gap")])
    ...
    # Summary statistics
    gap_stats = (
        gap_df.group_by(["instrument", "sample_type"])
        .agg(
            [
    ...
    # Histogram chart
    gap_chart = (
        alt.Chart(gap_df.filter(pl.col("gap") <= 15).to_pandas())
        ...
        .properties(width=150, height=150, title="Gap Distribution Between Consecutive QC/Clean Samples")
    )
...
def _(alt, df, mo, pl):
    ...
    _non_samples = df.filter(pl.col("sample_type").is_in(["qc", "clean", "blank"])).sort(
        ["instrument", "source_file", "run"]
    )
    ...
    _pair_chart = (
        ...
        .properties(width=120, height=200, title="Consecutive Pairs (run N → run N+1)")
    )
