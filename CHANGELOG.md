...
- Randomization mode `blocked_uniform`: spreads each `grouping_var` group evenly across the whole queue (fair-share interleave) instead of front-loading complete blocks like `blocked`, avoiding majority-only tail injections for unbalanced group counts; the group order within each fair-share round is randomized (seeded), so equal-sized groups no longer follow a fixed repeating sequence.
