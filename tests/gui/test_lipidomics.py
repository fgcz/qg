"""Scenario T15 — Lipidomics queue download verifies pos/neg polarity expansion.

The Lipidomics technology (like Metabolomics) splits each user sample into one
positive- and one negative-polarity injection. The test downloads the CSV and
asserts that each of the eight Lip_NN samples appears in two rows whose
``File Name`` carries ``_pos`` and ``_neg`` suffixes respectively.
"""
...
scenarios("features/lipidomics.feature")
...
@then("every Lipidomics sample appears in both polarities")
